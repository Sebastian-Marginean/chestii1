...
const PrioritateUrgenta = ({ id, setisModalNewTaskOpen }: Props) => {
  const {
    data: tasks,
    error,
    isLoading,
  ...
  // Filtrare doar taskuri cu prioritate "Urgenta"
  const urgente = (tasks || []).filter(
    (task: any) => task.prioritate?.toLowerCase() === "urgenta",
  );
  ...
  const columns: GridColDef[] = [
        ...
        <span
          className={
            ...
            (params.value === "Backlog"
              ...
              : params.value === "In Progres"
                ? "bg-blue-100 text-blue-800"
                : params.value === "Finalizat"
                  ? "bg-green-100 text-green-800"
                  : params.value === "Blocate"
    ...
    { field: "prioritate", headerName: "Prioritate", width: 100 },
  ...
  if (isLoading) return <div>Se incarcă...</div>;
  if (error) return <div>A apărut o eroare la preluarea sarcinilor</div>;
    ...
    <div className="h-[540px] w-full rounded-2xl border-4 border-indigo-200 bg-gradient-to-br from-indigo-100 via-blue-50 to-white px-4 pb-8 shadow-2xl xl:px-6">
      <div className="flex items-center pb-2 pt-5">
        <div className="flex-1 bg-gradient-to-r from-indigo-600 via-blue-500 to-purple-500 bg-clip-text text-transparent drop-shadow-lg">
          <Header nume="Taskuri cu Prioritate Urgentă" textMic />
        </div>
        <button
          className="ml-auto flex items-center gap-2 rounded-lg bg-gradient-to-r from-blue-600 via-indigo-500 to-purple-500 px-4 py-2 font-semibold text-white shadow-lg transition-all hover:scale-110 hover:brightness-125 focus:outline-none focus:ring-2 focus:ring-blue-400"
...
export default PrioritateUrgenta;
